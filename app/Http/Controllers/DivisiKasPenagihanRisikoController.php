...
class DivisiKasPenagihanRisikoController extends Controller
{
    private function checkRole()
    {
        if (auth()->user()->role !== 'divisi_kas_penagihan') {
    ...
    public function index()
    {
        ...
        $alerts = DaftarRisiko::where('unit_nama', 'Divisi Kas & Penagihan')
            ->whereNotNull('feedback_admin')
            ->where('feedback_admin', '!=', '')
            ->get();

        $risiko = DaftarRisiko::where('unit_nama', 'Divisi Kas & Penagihan')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        // Tambahkan 'alerts' ke dalam compact
        return view('divisi_kas_penagihan_risiko.daftar_risiko', compact('risiko', 'alerts'));
    }

    public function create()
    {
        $this->checkRole();
        return view('divisi_kas_penagihan_risiko.tambah_risiko');
    }
    ...
    public function store(Request $request)
    {
        ...
        $risiko->unit_nama = 'Divisi Kas & Penagihan';
        ...
        return redirect()
            ->route('divisi_kas_penagihan.risiko.index')
    ...
    public function edit($id)
    {
        ...
        return view('divisi_kas_penagihan_risiko.edit_risiko', compact('risiko'));
    }
