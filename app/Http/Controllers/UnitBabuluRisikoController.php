...
class UnitBabuluRisikoController extends Controller
{
    private function checkRole()
    {
        if (auth()->user()->role !== 'unit_Babulu') {
    ...
    public function index()
    {
        ...
        $alerts = DaftarRisiko::where('unit_nama', 'Unit Babulu')
            ->whereNotNull('feedback_admin')
            ->where('feedback_admin', '!=', '')
            ->get();

        $risiko = DaftarRisiko::where('unit_nama', 'Unit Babulu')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        // Tambahkan 'alerts' ke dalam compact
        return view('unit_babulu_risiko.daftar_risiko', compact('risiko', 'alerts'));
    }

    public function create()
    {
        $this->checkRole();
        return view('unit_babulu_risiko.tambah_risiko');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('unit_babulu.risiko.index')
    ...
    public function edit($id)
    {
        ...
        return view('unit_babulu_risiko.edit_risiko', compact('risiko'));
    }
