...
class DivisiPembukuanRisikoController extends Controller
{
    private function checkRole()
    {
        if (auth()->user()->role !== 'divisi_pembukuan') {
    ...
    public function index()
    {
        ...
        $alerts = DaftarRisiko::where('unit_nama', 'Divisi Pembukuan')
            ->whereNotNull('feedback_admin')
            ->where('feedback_admin', '!=', '')
            ->get();

        $risiko = DaftarRisiko::where('unit_nama', 'Divisi Pembukuan')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        // Tambahkan 'alerts' ke dalam compact
        return view('divisi_pembukuan_risiko.daftar_risiko', compact('risiko', 'alerts'));
    }

    public function create()
    {
        $this->checkRole();
        return view('divisi_pembukuan_risiko.tambah_risiko');
    }
    ...
    public function store(Request $request)
    {
        ...
        $risiko->unit_nama = 'Divisi Pembukuan';
        ...
        return redirect()
            ->route('divisi_pembukuan.risiko.index')
    ...
    public function edit($id)
    {
        ...
        return view('divisi_pembukuan_risiko.edit_risiko', compact('risiko'));
    }
