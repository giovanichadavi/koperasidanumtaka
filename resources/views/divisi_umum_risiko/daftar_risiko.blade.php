...
<style>
    ...
    /* --- PERBAIKAN TOMBOL AKSI: SEJAJAR & IDENTIK --- */
    .action-container {
        display: flex;
        flex-direction: row;
        justify-content: center;
        align-items: center;
        gap: 10px; /* Jarak antar tombol yang lebih lega */
        min-width: 200px; /* Kolom aksi yang pas (tidak terlalu lebar/sempit) */
    }

    .form-inline-action { display: inline-block; margin: 0; padding: 0; }

    .btn-action {
        width: 85px; /* UKURAN IDENTIK: Lebar disamakan */
        height: 32px; /* UKURAN IDENTIK: Tinggi disamakan */
        display: inline-flex;
        align-items: center;
        justify-content: center;
...
<div class="card mt-2 shadow-sm">
    <div class="card-header py-3 border-0 d-flex align-items-center">
        <h5 class="mb-0 flex-grow-1 text-center font-weight-bold text-uppercase">Laporan Daftar Risiko Divisi Umum</h5>
        <a href="{{ route('divisi_umum.risiko.create') }}" class="btn btn-primary btn-sm shadow-sm">
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="action-container">
                                <form action="{{ route('divisi_umum.risiko.destroy', $r->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?')" class="form-inline-action">
                                    @csrf @method('DELETE')
                                ...
                                <a href="{{ route('divisi_umum.risiko.edit', $r->id) }}" class="btn btn-warning btn-action shadow-sm">
