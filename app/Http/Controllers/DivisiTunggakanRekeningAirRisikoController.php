...
class DivisiTunggakanRekeningAirRisikoController extends Controller
{
    private function checkRole()
    {
        if (auth()->user()->role !== 'divisi_tunggakan_rekening_air') {
    ...
    public function index()
    {
        ...
        $alerts = DaftarRisiko::where('unit_nama', 'Divisi Tunggakan Rekening Air')
            ->whereNotNull('feedback_admin')
            ->where('feedback_admin', '!=', '')
            ->get();

        $risiko = DaftarRisiko::where('unit_nama', 'Divisi Tunggakan Rekening Air')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        // Tambahkan 'alerts' ke dalam compact
        return view('divisi_tunggakan_rekening_air_risiko.daftar_risiko', compact('risiko', 'alerts'));
    }

    public function create()
    {
        $this->checkRole();
        return view('divisi_tunggakan_rekening_air_risiko.tambah_risiko');
    }
    ...
    public function store(Request $request)
    {
        ...
        $risiko->unit_nama = 'Divisi Tunggakan Rekening Air';
        ...
        return redirect()
            ->route('divisi_tunggakan_rekening_air.risiko.index')
    ...
    public function edit($id)
    {
        ...
        return view('divisi_tunggakan_rekening_air_risiko.edit_risiko', compact('risiko'));
    }
