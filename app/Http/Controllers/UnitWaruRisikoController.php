...
class UnitWaruRisikoController extends Controller
{
    private function checkRole()
    {
        if (auth()->user()->role !== 'unit_waru') {
    ...
    public function index()
    {
        ...
        $alerts = DaftarRisiko::where('unit_nama', 'Unit Waru')
            ->whereNotNull('feedback_admin')
            ->where('feedback_admin', '!=', '')
            ->get();

        $risiko = DaftarRisiko::where('unit_nama', 'Unit Waru')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        // Tambahkan 'alerts' ke dalam compact
        return view('unit_waru_risiko.daftar_risiko', compact('risiko', 'alerts'));
    }

    public function create()
    {
        $this->checkRole();
        return view('unit_waru_risiko.tambah_risiko');
    }
    ...
    public function edit($id)
    {
        ...
        return view('unit_waru_risiko.edit_risiko', compact('risiko'));
    }
